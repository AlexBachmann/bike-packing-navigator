...
2. **`guidance-track.json`** (Enriched): Road- and trail-snapped polyline coordinates aligned with OSM way centerlines within 50m, falling back to raw GPX for backcountry (>50m). Generated by `snap_route_to_osm.py` when `corridor.pmtiles` is present.
3. **`surfaces.json`**: Contiguous surface intervals `[start_km, end_km, highway, surface, tracktype]`.
